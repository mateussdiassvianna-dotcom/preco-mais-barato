...
ADMIN_USER = {
    ...
    "senha": os.getenv("ADMIN_PASSWORD"),  # em produção, criptografe!
}
...
def admin_dashboard():
    ...
    try:
        total_comerciantes = len(
            supabase.table("comerciantes").select("id").execute().data or []
        )
        aprovados = len(
        ...
        pendentes = len(
            supabase.table("comerciantes_pendentes").select("id").execute().data or []
        )
        total_produtos = len(
            supabase.table("produtos").select("id").execute().data or []
        )
        total_pesquisas = len(
            supabase.table("pesquisas").select("id").execute().data or []
        )
        total_acessos = len(
            supabase.table("historico_comerciantes").select("id").execute().data or []
        )

        pendentes_list = (
            supabase.table("comerciantes_pendentes").select("*").execute().data or []
        )
        ...
        return render_template(
            "admin_dashboard.html", stats=stats, pendentes=pendentes_list
        )
...
def admin_comerciantes():
    ...
    try:
        ...
        for c in comerciantes:
            ...
            historico = (
                supabase.table("historico_comerciantes")
                .select("*")
                .eq("comerciante_id", cid)
...
def admin_bloquear_comerciante(id):
    ...
    try:
        ...
        comerciante = (
            supabase.table("comerciantes").select("*").eq("id", id).execute().data
        )
        if not comerciante:
        ...
        supabase.table("comerciantes").update({"status": "bloqueado"}).eq(
            "id", id
        ).execute()
...
def admin_desbloquear_comerciante(id):
    ...
    try:
        ...
        supabase.table("comerciantes").update({"status": "ativo"}).eq(
            "id", id
        ).execute()
...
def admin_apagar_comerciante(id):
    ...
    try:
        ...
        supabase.table("historico_comerciantes").delete().eq(
            "comerciante_id", id
        ).execute()
...
def admin_produto_detalhes(id):
    ...
    try:
        ...
        comerciante = (
            supabase.table("comerciantes")
            .select("*")
            .eq("id", p["comerciante_id"])
            .execute()
            .data
        )
        p["comerciante_nome"] = (
            comerciante[0]["nome"] if comerciante else "Desconhecido"
        )
...
def admin_pesquisas():
    ...
    try:
        ...
        for p in pesquisas:
            ...
            produtos_count[produto_nome] = (
                produtos_count.get(produto_nome, 0) + qtd_pesquisas
            )
            clicados_count[produto_nome] = (
                clicados_count.get(produto_nome, 0) + qtd_cliques
            )
            comerciantes_count[comerciante_nome] = (
                comerciantes_count.get(comerciante_nome, 0) + qtd_pesquisas
            )
        ...
        comerciante_mais_pesquisado = max(
            ...
            key=lambda x: x["count"],
            default={"nome": "N/A", "count": 0},
        )

        # 🔹 Produto mais pesquisado e clicado
        produto_mais_pesquisado = (
            top_produtos_pesquisados[0]
            if top_produtos_pesquisados
            else {"nome": "N/A", "count": 0}
        )
        produto_mais_clicado = (
            top_produtos_clicados[0]
            if top_produtos_clicados
            else {"nome": "N/A", "count": 0}
        )
        ...
        comerciantes = (
            supabase.table("comerciantes").select("nome").execute().data or []
        )
...
def aprovar_comerciante(id):
    ...
    try:
        ...
        if not pendente:
            return (
                jsonify(
                    {"sucesso": False, "erro": "Comerciante pendente não encontrado"}
                ),
                404,
            )
